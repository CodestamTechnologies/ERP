...
// Enhanced mock data for the dedicated reconciliation page
const MOCK_STATEMENTS: ReconciliationStatement[] = [
  {
    id: 'stmt-001',
    accountId: 'acc-001',
  ...
  },
  {
    ...
    fileName: 'deutsche_statement_feb_2024.xlsx',
    fileSize: 3072000
  }
];
...
const MOCK_RULES: ReconciliationRule[] = [
  {
    id: 'rule-001',
...
export const useReconciliationPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setReconciliationItems(MOCK_DISCREPANCIES);
      setRules(MOCK_RULES);
      setLoading(false);
    };
  ...
  const summary = useMemo((): ReconciliationSummary => {
    ...
    const averageReconciliationTime = 4.5;
  ...
  const resolveDiscrepancy = useCallback(async (itemId: string, resolution: string) => {
