// hooks/useSalesFirestore.ts
import { useState, useEffect, useCallback } from 'react';
import { 
  getOrdersFirestore, 
  getSalesTeamFirestore, 
  getOrdersAnalyticsFirestore,
  addOrderFirestore, 
  updateOrderFirestore, 
  deleteOrderFirestore,
  addTeamMemberFirestore,
  updateTeamMemberFirestore,
  deleteTeamMemberFirestore,
  formatIndianCurrency
} from '@/lib/firebase/database/useSalesFirestore';
import { 
...
export const useSalesFirestore = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = getOrdersFirestore((ordersData) => {
  ...
  // Load team members with Firestore
  useEffect(() => {
    setLoading(prev => ({ ...prev, team: true }));
    
    const unsubscribe = getSalesTeamFirestore((teamData) => {
  ...
  // Load activities with Realtime Database (keeping existing implementation)
  useEffect(() => {
  ...
  // Load analytics with Firestore
  useEffect(() => {
    setLoading(prev => ({ ...prev, analytics: true }));
    
    const unsubscribe = getOrdersAnalyticsFirestore((analyticsData) => {
  ...
  const handleAddOrder = async (orderData: Omit<SalesOrder, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      await addOrderFirestore(orderData);
  ...
  const handleUpdateOrder = async (id: string, updates: Partial<SalesOrder>) => {
    try {
      await updateOrderFirestore(id, updates);
      // Orders will update automatically via Firestore listener
    } catch (error) {
  ...
  const handleAddTeamMember = async (memberData: Omit<SalesTeamMember, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      await addTeamMemberFirestore(memberData);
      // Team members will update automatically via Firestore listener
    } catch (error) {
  ...
  const handleUpdateTeamMember = async (id: string, updates: Partial<SalesTeamMember>) => {
    try {
      await updateTeamMemberFirestore(id, updates);
      // Team members will update automatically via Firestore listener
    } catch (error) {
  ...
  const handleDeleteTeamMember = async (id: string) => {
    try {
      await deleteTeamMemberFirestore(id);
      // Team members will update automatically via Firestore listener
    } catch (error) {
